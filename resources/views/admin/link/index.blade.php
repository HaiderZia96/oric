...
@section('content')
    <div class="container-fluid">
        <div class="d-flex justify-content-between py-4">
            <p class="page-title">Link</p>
            <a class="btn btn-sm btn-success" href="{{route('link.create')}}">Add New</a>
        </div>
            ...
            var t = $('.dataTable').DataTable({
                ...
                columnDefs: [
                    ...
                    },
                    {
                        ...
                        // },
                        render: function (data,type,row,meta) {
                            var edit ='{{ route("link.edit", ":id") }}';
                            edit = edit.replace(':id', data.id);
                            var del ='{{ route("link.delete", ":id") }}';
                            del = del.replace(':id', data.id);
                            var sdel ='{{ route("link.destroy", ":id") }}';
                            sdel = sdel.replace(':id', data.id);
                            var restore ='{{ route("link.restore", ":id") }}';
